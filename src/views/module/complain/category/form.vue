    ...
    <div class="admin_main_block">
      <div class="admin_main_block_top">
        <div class="admin_main_block_left">
          <div>{{ $t('complain.category.from') }}</div>
        </div>
      ...
      <div class="admin_form_main">
        <el-form label-width="120px" ref="dataForm" :model="dataForm" :rules="dataRule">
          <el-form-item :label="$t('complain.category.title')" prop="title">
  ...
  export default
  {
    ...
    data()
    {
      return {
        ...
        {
          title: [
            { required: true, message: this.$t('complain.category.rules.title.require'), trigger: 'blur' },
          ]
        }
      };
    },
    methods:
    {
      init ()
      {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/complain/category/view/${this.dataForm.id}`),
              method: 'get',
      ...
      dataFormSubmit () {console.log(1);
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/complain/category/handle`),
              method: 'post',
